...
cv_entries <- read_excel("cv_data.xlsx", sheet = "cv_entries") |> 
  mutate(year_end = if_else(date_end == "present", "present", str_sub(date_end, start = 1, end = 4)),
         year = if_else(!is.na(year_end), str_c(year(date), " --- ", year_end), as.character(year(date))))
...
cv_entries |> 
  ...
  blank_year_when_repeated() |> 
  select(year, number, what) |> 
  manual_page_break_after_row("what", "Kynapse") |> 
  manual_page_break_after_row("what", "Seminars at Linacre College") |> 
  manual_page_break_after_row("what", "Minimally-Distracting") |> 
  knitr::kable()
...
cv_entries |> 
  ...
  mutate(what = glue("<span class='cv-entry-title'>{what}</span>, {where}<br>",
         "{additional_info}")) |> 
  select(year, what) |> 
  manual_page_break_after_row("what", "2-day master class in reproducible research") |> 
  kable()
```
