...
Since 2016, I have shared materials, analysis scripts, and data for all my work, and written my papers as reproducible documents in [R Markdown](https://rmarkdown.rstudio.com).
:::
:::
...
cv_entries %>%
  ...
  mutate(what = case_when(
    is.na(additional_info) ~ str_c("<span class='award-what'>", what, "</span> (", amount, "), ", where),
    !is.na(additional_info) ~ str_c("<span class='award-what'>", what, "</span> (", amount, "), ", where, ".<br> ", additional_info),
  )) %>%
  mutate(row_number = row_number()) %>%
  mutate(year_begin = as.character(year_begin),
         year_begin = case_when(
...
publications_table <- publications %>%
  ...
  mutate(citation = case_when(
    !is.na(award) & str_detect(award, "honourable") ~ str_c("<span class='publication-title'>", title, "</span><br><span class='publication-authors'>", authors_full, "</span><br><span class='publication-venue'>", venue_abbrev, ": ", venue, "</span><br><i class='fas fa-award'></i> <span class='publication-award'>", award, "</span>"),
    !is.na(award) ~ str_c("<span class='publication-title'>", title, "</span><br><span class='publication-authors'>", authors_full, "</span><br><span class='publication-venue'>", venue_abbrev, ": ", venue, "</span><br><i class='fa fa-trophy'></i> <span class='publication-award'>", award, "</span>"),
    TRUE ~ str_c("<span class='publication-title'>", title, "</span><br><span class='publication-authors'>", authors_full, "</span><br><span class='publication-venue'>", venue_abbrev, ": ", venue, "</span>")
    )) %>% 
...
## Selected work experience {.teaching}
```{r}
cv_entries %>% 
