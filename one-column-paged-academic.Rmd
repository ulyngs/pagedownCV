...
:::
<!-- # ADD SECTION W/ 'Publicly available research code & data' like Matt Kay -->
...
cv_entries %>% 
  ...
  mutate(what = case_when(
    ...
    !is.na(additional_info) ~ str_c("<span class='award-what'>", what, "</span> (", amount, ")<br>", where, "<br>", additional_info),
  )) %>%
  mutate(row_number = row_number()) %>% 
  mutate(year_begin = as.character(year_begin),
         year_begin = case_when(
...
<div class="grid-item conference-publication-heading">
<h3>Conference publications <br><span class="review-note">(fully reviewed, archival)</span></h3>
</div>
<div class="grid-item conference-note">
<h3>In computer science, top-tier conferences (<30% acceptance rate) are as, or more impactful than journals, see http://doi.org/fgjt2h</h3>
</div>
:::
```{r}
...
cv_entries %>% 
  ...
  arrange(date_begin) %>% 
  mutate(number = str_c("P", row_number())) %>% 
...
cv_entries %>% 
  ...
  arrange(date_begin) %>% 
  mutate(number = str_c("T", row_number())) %>% 
  arrange(desc(date_begin)) %>% 
  mutate(row_number = row_number()) %>% 
  mutate(year_begin = as.character(year_begin),
         year_begin = case_when(
