...
publications_formatted |>
  ...
  blank_year_when_repeated() |>
  select(year, number, citation) |>
  manual_page_break_after_row("citation", "Further Exploring") |> 
  knitr::kable()
...
pubs_w_number <- publications |> 
  select(type, authors, date, title, materials) |> 
...
public_code |> 
  ...
  select(year, what) |> 
  blank_year_when_repeated() |> 
  manual_page_break_after_row("what", "Self-Control in Cyberspace") |> 
  kable()
```
...
cv_entries |> 
  ...
  select(year, what) |> 
  manual_page_break_after_row("what", "Oxford LaTeX template") |> 
  kable()
```
...
cv_entries |> 
  ...
  mutate(what = glue("<span class='cv-entry-title'>{what}</span>, {where}<br>",
                     "{additional_info}")) |> 
  select(year, what) |>
  manual_page_break_after_row("what", "practical support and consultancy on natural language") |> 
  kable()
```
